    ...
    <div>
        ...
        <ul>
            <li v-for="marvelComic in filteredComics" :key="marvelComic.id">
                <a href="#" type="button"><img :src="`${marvelComic.thumbnail.path}.${marvelComic.thumbnail.extension}`"
                        :alt="marvelComic.title" class="comic-img" />
                    {{ marvelComic.title }}<button :key="marvelComic.id"
                        @click="() => { TogglePopup('buttonTrigger') }">Show</button>
